...
class Animation(object):
    def __init__(self, frames, framerate = constants.ANIM_FRAMERATE):
        # Constructor function for the animation object. Starts off in the STOPPED state.
        #
        # @param frames
        #     A list of surfaces or strings for each frame of animation, in one of the following format:
        #       image_of_frame<pygame.Surface>
        #       filename_of_image<str>
        ...
        if frames != '_copy': # ('_copy' is passed for frames by the getCopies() method)
            ...
            for i in range(self.numFrames):
                ...
                frame = frames[i]
                assert type(frame) in (str, pygame.Surface), 'Frame %s image must be a string filename or a pygame.Surface' % (i)
                if type(frame) == str:
